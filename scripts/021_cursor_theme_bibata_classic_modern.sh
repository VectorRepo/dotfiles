...
main() {
    ...
    # 2. Apply via gsettings (Niri / GNOME-compatible)
    log_info "Applying cursor theme via gsettings..."
    if gsettings set org.gnome.desktop.interface cursor-theme "${THEME_NAME}" && \
       gsettings set org.gnome.desktop.interface cursor-size "${CURSOR_SIZE}"; then
        log_success "Cursor theme active: ${THEME_NAME} @ ${CURSOR_SIZE}px"
    else
        log_warn "gsettings apply failed. Theme installed but not active until relogin."
    fi
}
